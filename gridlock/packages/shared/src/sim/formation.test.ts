...
describe("groupMoveTargets", () => {
  ...
  it("spreads stacked units by reserved radius around the click", () => {
    ...
    const t2 = makeEntity(state, "rifleman", "A", 20 * 32, 20 * 32);
    ...
    assertSpaced(pts);
    const reach = unitClearance(catalog("warden").radius, catalog("rifleman").radius) + 8;
    for (const p of pts) {
  ...
  it("keeps a spaced line's relative layout", () => {
    const { state } = twoPlayerMatch();
    const a = makeEntity(state, "rifleman", "A", 16 * 32, 16 * 32);
    const b = makeEntity(state, "rifleman", "A", 16 * 32, 16 * 32 + 40);
  ...
  it("uses a larger gap when a Warden is in the group", () => {
    ...
    assert.ok(gap + 1e-6 >= unitClearance(catalog("rifleman").radius, catalog("warden").radius));
  });
});
...
describe("cmd.move group", () => {
  it("does not stack selected units on the click", () => {
    ...
    const t1 = makeEntity(state, "rifleman", "A", tileCenter(36, ts), tileCenter(16, ts));
    const t2 = makeEntity(state, "rifleman", "A", tileCenter(36, ts), tileCenter(16, ts));
  ...
  it("marches mixed units at the slowest catalog speed", () => {
    ...
    const inf = makeEntity(state, "rifleman", "A", tileCenter(16, ts), tileCenter(20, ts));
    const tank = makeEntity(state, "warden", "A", tileCenter(16, ts), tileCenter(28, ts));
    const solo = makeEntity(state, "rifleman", "A", tileCenter(16, ts), tileCenter(10, ts));
  ...
  it("stamps the same pace on attack-move", () => {
    const { state } = twoPlayerMatch();
    const inf = makeEntity(state, "rifleman", "A", 20 * 32, 20 * 32);
...
describe("groupMovePace", () => {
  it("caps a mixed selection to the slowest walker", () => {
    const { state } = twoPlayerMatch();
    const inf = makeEntity(state, "rifleman", "A", 100, 100);
  ...
  it("leaves a solo unit and a same-type group uncapped", () => {
    const { state } = twoPlayerMatch();
    const a = makeEntity(state, "rifleman", "A", 100, 100);
    const b = makeEntity(state, "rifleman", "A", 140, 100);
